...
const LeaveRequestsPage: React.FC = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            setToast({ message: 'خطا در دریافت درخواست‌ها', type: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md mb-8 flex justify-between items-center">
                ...
                <button onClick={() => { setCurrentRequest({ type: 'DAILY', status: 'PENDING' }); setIsModalOpen(true); }} className="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 flex items-center gap-2">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {requests.map(req => (
                                    <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-slate-800 dark:text-slate-200">
                                        <td className="p-4 font-bold">{req.requesterName}</td>
                                ...
                                {requests.length === 0 && (
                                    <tr><td colSpan={6} className="p-8 text-center text-slate-400">هیچ درخواستی وجود ندارد.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-white dark:bg-slate-800 rounded-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-4">
                            <input type="text" placeholder="نام متقاضی" className="w-full px-4 py-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={currentRequest.requesterName || ''} onChange={e => setCurrentRequest({...currentRequest, requesterName: e.target.value})} />
